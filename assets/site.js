...
Promise.all([fresh('repo-config.json'), fresh('packages.json')]).then(([config, packages])=>{
  ...
  $('packages').innerHTML=packages.map(p=>`<a class="package-card" href="package.html?id=${encodeURIComponent(p.id)}">${p.iconImage?`<img class="tweak-icon" src="${p.iconImage}" alt="${p.name} icon">`:`<i style="--icon:${p.iconColor||s.accent}">${p.iconText||p.name.slice(0,2)}</i>`}<span>${p.compatibility||p.architecture}</span><h3>${p.name}</h3><p>${p.tagline||p.summary}</p><div><b>${p.version}</b><strong>View details →</strong></div></a>`).join('');
